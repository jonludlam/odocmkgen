...
let to_sexp v =
  ...
  let ps = S.fold (fun package cur -> 
    (List [ Atom package.Opam.name; Atom package.Opam.version ]) :: cur) v.packages [] in
  List [
    List [ Atom v.id ];
    List ps
  ]
...
let of_string str =
  ...
  | _ -> failwith "bad sexp"
  in
  let packs = List.map (function | Sexplib.Sexp.List [ Atom name; Atom version ] -> { Opam. name; version } | _ -> failwith "bad sexp") packs_s in
