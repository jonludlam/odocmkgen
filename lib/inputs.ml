...
let with_open_dir d f =
  ...
  Util.protect ~finally:(fun () -> Unix.closedir dh) (fun () -> f dh)

let filter_dots = function "." -> [] | ".." -> [] | e -> [ e ]

let contents dir =
  with_open_dir dir (fun dh ->
      let rec loop acc = try loop (Unix.readdir dh :: acc) with _ -> acc in
      loop [])
  >>= filter_dots
  >>= fun x -> [ Fpath.(dir / x) ]
...
let has_ext exts f = List.exists (fun suffix -> Fpath.has_ext suffix f) exts

let rec find_files extensions base_dir =
  let items = contents base_dir in
  let dirs = items >>= filter is_dir in
  let cmis =
    items >>= filter (has_ext extensions) |> List.map Fpath.rem_ext |> setify
  in
