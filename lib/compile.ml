  ...
  | Packages ->
      ...
      let interpose_alphabet packages =
        ...
        let rec inner ps =
          ...
          | a :: b :: rest ->
              let na = name a in
              let nb = name b in
              if Char.uppercase_ascii na.[0] <> Char.uppercase_ascii nb.[0] then
      ...
      @ interpose_alphabet
          (List.sort
             (fun c1 c2 ->
               String.compare
                 (String.uppercase_ascii (name c1))
                 (String.uppercase_ascii (name c2)))
             children)
  | Package p ->
...
module PandU = struct
  ...
  let equal (p1, d1) (p2, d2) =
    Digest.equal d1 d2 && p1.Opam.name = p2.Opam.name && p1.version = p2.version
    ...
    (fun (package, universe_id) deps acc ->
      ...
      let extra =
        ...
        if List.length missing > 0 then
          ...
          List.filter_map
            (fun x -> match x with Ok x -> Some x | _ -> None)
            extra
        else []
      in
    ...
    List.fold_left
      (fun map info ->
        StringMap.update info.digest
          (function None -> Some [ info ] | Some xs -> Some (info :: xs))
          map)
